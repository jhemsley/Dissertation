...
# size and peak.rate are strongly correnated, so which makes a better model?
fit <-  lm(alpha ~ closeness.L + w.size.L + peak.at.L + occ.time.lh + occ.time.rh, data=rtes) 
fit <-  lm(alpha ~ closeness.L + peak.rate.L + peak.at.L + occ.time.lh + occ.time.rh, data=rtes) 

fit <-  lm(alpha ~ closeness.L + peak.rate.L + peak.at.L + occ.t.lh + occ.t.rh, data=rtes) 
#fit <-  lm(alpha ~ closeness.L + peak.at.L + occ.t.lh + occ.t.rh, data=rtes) 
#fit <-  lm(alpha ~ closeness.L + peak.rate.L + peak.at.L + octm.1 + octm.2 + octm.3, data=rtes) 

#fit <-  lm(log(alpha) ~ closeness.L + peak.rate.L + peak.at.L + follower.start.L + mean.followers.L + overlap.sqrt + occ.t.lh + occ.t.rh, data=rtes) 

fit.2 <-  lm(alpha ~ closeness.L + peak.rate.L + peak.at.L + poly(occ.peak.time.marker,2), data=rtes) 
...
# 3 diagnostic plots
#
#plot.file.name.png <- paste(regression.plot.dir, "RegressionDiagPlots_Alpha.png", sep="")
#png(plot.file.name.png, width=625, height=350, pointsize=16)
plot.file.name.pdf <- paste(regression.plot.dir, "RegressionDiagPlots_Alpha.pdf", sep="")
pdf(plot.file.name.pdf, width=6.25, height=3.50, pointsize=16, useDingbats = FALSE)
  ...
  abline(h=0)
  mtext(text="Model 1", side=1, line=2, adj=-.5)
  qqPlot(fit, main="QQ Plot", lwd=1, cex=.7, col=rgb(64, 153, 255, maxColorValue=255))
dev.off()  


plot.file.name.png <- paste(regression.plot.dir, "RegressionLeveragePlots_Alpha.png", sep="")
png(plot.file.name.png, width=625, height=350, pointsize=16)
...
boxcox(fit, lambda=seq(-1,1,by=.01), plotit=T)
confint(fit)
