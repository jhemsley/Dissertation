...
for (network.choice in 1:8) {
  ...
  ###############################
  #plot.community.detection <- FALSE
  if (plot.community.detection) {
    tmp.layout.name <- "community.detection"
    ...
    cat(network.name, " (", network.choice, "): Plotting community detection to ", net.plot.f.name, ", as PDF: ", plot.to.pdf, ", at ", as.character(Sys.time()), "\n", sep="")
    if (plot.to.pdf) {
      ...
      par(mar=c(3, 1, 4, 1))
      plot(fc, g, layout=layout.kamada.kawai, vertex.color="white", vertex.size=1.5, vertex.label="", edge.arrow.size=0, edge.arrow.width=0, main="")
      mtext(text=paste(network.name, "communities", sep=" "), side=3, col="white")
      mtext(text="spinglass.community detection", side=1, col="pink")
      
      par(mar=c(4, 6, 1, 4))
      plot(sizes(fc), bty="l", main="community sizes")
    
    dev.off()
    #wc <- walktrap.community(g)
    #plot(wc, g, vertex.size=1, vertex.label="", edge.arrow.size=0, edge.arrow.width=0, main=paste(network.name, "communities", sep=" "))
    #mtext(text="walktrap.community detection", side=1)
    #plot(sizes(wc), bty="l", main="community sizes")
  ...
  # plot.bipartite <- FALSE
  if (plot.bipartite) {    
    tmp.layout.name <- "bipartite"
    bipart.edgelist.rt <- data.frame(from=tweet.data$rt.user.screen_name, to=tweet.data$rt.id_str)  
    
    bipart.g <- graph.data.frame(bipart.edgelist.rt, directed=TRUE)
    bipart.g <- simplify(bipart.g, edge.attr.comb="sum")
    bipart.el <- get.edgelist(bipart.g) # need this here for later. Don't simplify network before calling this
    class(bipart.el)
    dim(bipart.el)
    
    bipart.edgelist <- data.frame(from=tweet.data$user.screen_name, to=tweet.data$rt.id_str)  
    bipart.g <- graph.data.frame(bipart.edgelist, directed=TRUE)
    bipart.el <- rbind(bipart.el, get.edgelist(bipart.g))
    dim(bipart.el)
    
    bipart.edgelist <- data.frame(from=bipart.el[,1], to=bipart.el[,2])  
    bipart.g <- graph.data.frame(bipart.edgelist, directed=TRUE)
    bipart.el[1:20,1]
    bipart.el[1:20,2]
    
    cat(network.name, " (", network.choice, "): starting kamada.kawai layout on bipartitie at ", as.character(Sys.time()), "\n")
    bipartite.layout <- layout.kamada.kawai(bipart.g)
    V(bipart.g)[V(bipart.g)$name %in% bipart.el[,2]]$color <- "red"
    V(bipart.g)[V(bipart.g)$name %in% bipart.el[,1]]$color <- "green"
    cat(network.name, " (", network.choice, "): DONE! ", as.character(Sys.time()), "\n")
    
    net.plot.f.name <- paste(dir.path.plots, network.name, "_", tmp.layout.name, "_", plot.width, "x", plot.height, bg.msg, sep="")
    net.plot.f.name
        
    cat(network.name, " (", network.choice, "): Plotting kamada.kawai layout on bipartitie to ", net.plot.f.name, ", as PDF: ", plot.to.pdf, ", at ", as.character(Sys.time()), "\n", sep="")
    if (plot.to.pdf) {
      pdf(paste(net.plot.f.name, ".pdf", sep=""), width=8.5, height=11, title=net.plot.f.name, pointsize=1/300, paper="letter", compress=F)
    } else {
      png(paste(net.plot.f.name, ".png", sep=""), width=plot.width, height=plot.height)
      ...
      t2 <- system.time(
        plot(bipart.g, vertex.size=1, vertex.label="", edge.arrow.size=0, edge.arrow.width=0)
      )
      tmp.msg <- paste(network.name, " bi-partitie", sep="")
      mtext(tmp.msg, side=3, line=2.5, cex=2, col=network.plot.fg.color)
      mtext("layout.kamada.kawai", side=1, line=-1, cex=2, col=network.plot.fg.color)
  ...
  #####################################
  #
  #  Network: Plot drl (openord) layouts for comparison 
  #
  #   Future plots: plot everthing but OccDenv, OccDenv neighbors, and 
  #                 links to and from occ denv. Next, plot only OccDenv
  #                 OccDenv neighbors and links to and from OccDenv.
  #                     To do this, use full transparency for for items
  #                     not to be plotted
  #
  ####################################
  
  #plot.width <- plot.height <- 1024
  
  if (plot.DrL.default.set) {
    for (i in 1:5) {
      
      # i <- 3
    
      if (i == 1) {
        tmp.drl.layout <- igraph.drl.default
        tmp.layout.name <- "default"
      } else if (i ==2) {
        tmp.drl.layout <- igraph.drl.coarsen
        tmp.layout.name <- "coarsen"
      } else if (i ==3) {
        tmp.drl.layout <- igraph.drl.coarsest
        tmp.layout.name <- "coarsest"
      } else if (i ==4) {
        tmp.drl.layout <- igraph.drl.refine
        tmp.layout.name <- "refine"
      } else if (i ==5) {
        tmp.drl.layout <- igraph.drl.final
        tmp.layout.name <- "final"
      }
      
      plot.title.2 <- paste("DrL param set: ", tmp.layout.name, sep="")
      plot.title.3 <- paste(num.nodes, " nodes, ", ecount(g), " edges", sep="")
    
      net.plot.f.name <- paste(dir.path.plots, network.name, "_", tmp.layout.name, "_", plot.width, "x", plot.height, bg.msg, sep="")
      net.plot.f.name
      
      cat(network.name, " (", network.choice, "): starting DrL option ", i, " at ", as.character(Sys.time()), "\n", sep="")
      l <- layout.drl(g, options=tmp.drl.layout)
      V(g)$x <- l[,1]
      V(g)$y <- l[,2]
      
      cat(network.name, " (", network.choice, "): Plotting DrL defaults to ", net.plot.f.name, ", as PDF: ", plot.to.pdf, ", at", as.character(Sys.time()), "\n", sep="")   
      if (plot.to.pdf) {
        pdf(paste(net.plot.f.name, ".pdf", sep=""), width=8.5, height=11, title=net.plot.f.name, pointsize=1/300, paper="letter", compress=F)
      } else {
        png(paste(net.plot.f.name, ".png", sep=""), width=plot.width, height=plot.height)
        ...
        plot.igraph(g, edge.arrow.size=0.02, edge.arrow.width=0.02, edge.curved = T
      )
    
      mtext(plot.title.1, side=3, line=2.5, cex=2, col=network.plot.fg.color)
      mtext(plot.title.2, side=3, line=1, cex=1.3, col=network.plot.fg.color)
      mtext(plot.title.3, side=3, line=-.7, cex=1.1, col=network.plot.fg.color)
    
      dev.off()
      cat("     device off at ", as.character(Sys.time()), "\n\n")
    }
    
  } # end plot.DrL.default.set
  ...
  if (plot.DrL.experimentation) {
    #igraph.drl.experiment <- igraph.drl.default
    #tmp.layout.name <- "experimentSimmer"
    tmp.layout.name <- "experimentInitFinal"
    igraph.drl.experiment <- igraph.drl.final
    
    igraph.drl.experiment$init.iterations # 0
    igraph.drl.experiment$liquid.iterations <- 10 # 0
    igraph.drl.experiment$expansion.iterations # 50
    igraph.drl.experiment$cooldown.iterations # 50
    igraph.drl.experiment$crunch.iterations <- 3 # 50
    igraph.drl.experiment$simmer.iterations <- 3 # 25
    
    # simmer seems to be the one that brings it all together at the end
    #igraph.drl.experiment$crunch.iterations <- 5 # 50
    #igraph.drl.experiment$simmer.iterations <- 500 #  2 # 100
    
    stage.col <- 
      c("darkblue"
        , rep("darkgoldenrod2", 4)
        , rep("darkslateblue", 4)
        , rep("darkorange", 4)
        , rep("darkgreen", 4)
        , rep("firebrick", 4)
    ...
    drl.params.vec <- rep("", 25)
    for (j in 1:25) {
      drl.params.vec[j] <- paste(names(igraph.drl.experiment)[j], ": ", igraph.drl.experiment[j], sep="")
    }
    ...
    cat(network.name, " (", network.choice, "): starting DrL experiment layout at ", as.character(Sys.time()), "\n")
    t <- system.time(
      l <- layout.drl(g, options=igraph.drl.experiment)
    )
    V(g)$x <- l[,1]
    V(g)$y <- l[,2]
    
    cat(network.name, " (", network.choice, "): Plotting DrL.experimentation to ", net.plot.f.name, ", as PDF: ", plot.to.pdf, ", at ", as.character(Sys.time()), "\n", sep="")
    if (plot.to.pdf) {
      pdf(paste(net.plot.f.name, ".pdf", sep=""), width=8.5, height=11, title=net.plot.f.name, pointsize=1/300, paper="letter", compress=F)
    } else {
      png(paste(net.plot.f.name, ".png", sep=""), width=plot.width, height=plot.height)
      ...
      t2 <- system.time(
        plot.igraph(g, edge.arrow.size=0.0, edge.arrow.width=0.00)
      )
      ...
      #mtext("Blue links: OccDenv RT others", side=3, line=2.3, cex=1, adj=.99, col=network.plot.fg.color)
      #mtext("Greens links: others RT OccDenv", side=3, line=1.1, cex=1, adj=.99, col=network.plot.fg.color)
    
      tmp.line <- -27
      for (j in 1:25) {
        mtext(drl.params.vec[j], side=1, line=tmp.line, col=stage.col[j], cex=.8, adj=0.01)
        tmp.line <- tmp.line + 1
      }
    
    dev.off()
    cat("     device off at ", as.character(Sys.time()), "\n\n")
  } # end plot.DrL.experimentation
